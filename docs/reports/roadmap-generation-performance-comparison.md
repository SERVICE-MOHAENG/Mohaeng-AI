...
개발/기본 설정의 `LLM_TIMEOUT_SECONDS` 기본값은 60초에서 180초로 변경하는 것을 제안한다.
...
현재 timeout policy는 `LLM_TIMEOUT_SECONDS`가 `REQUEST_TIMEOUT_SECONDS` 상한을 넘지 못하므로 두 값을 함께 조정해야 한다.
...
후속 조치는 추가 최적화보다 timeout 기본값을 180초로 변경하고, 프로덕션에서는 300초를 사용하는 방향이 현실적이다.
