...
현재 지원하는 수정 작업은 장소 교체, 추가, 삭제, 같은 일자 내 순서 이동, 일차 간 장소 이동, 일차 일정 묶음 교체입니다.
...
일차 간 MOVE와 일차 일정 묶음 교체처럼 여러 일차가 바뀌는 경우, `diff_keys`에서 추출한 모든 일차에 같은 정책을 적용합니다.
